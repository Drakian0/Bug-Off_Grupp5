...
public class TestDecaDiscusThrow {
    ...
    Collection<DynamicTest> dynamicTestsForCalculateResult() {
        ...
        double maxDistance = 85.0; // Hardcoded for now, perhaps better to use a getter to get it from the class file incase changes are made

        BigDecimal start = new BigDecimal(minDistance); // "0.00"
        BigDecimal end = new BigDecimal(maxDistance); // "85.00"
        ...
        for (BigDecimal distance = start; distance.compareTo(end) <= 0; distance = distance.add(increment)) {
            ...
            Executable exec = () -> {
                ...
                // Inject the mock into DecaDiscusThrow
                decaDiscusThrow.inputResult = mockInputResult;

                // Set active to true to ensure the loop runs as expected
                decaDiscusThrow.setActive(true);

                double A = decaDiscusThrow.getA();
                double B = decaDiscusThrow.getB();
                double C = decaDiscusThrow.getC();

                int expectedScore = calculateExpectedScore(finalDistance, A, B, C);
                int actualScore = decaDiscusThrow.calculateResult(finalDistance);
