...
public class TestDeca100M {
    ...
    Collection<DynamicTest> dynamicTestsForCalculateResult() {
        ...
        double minTime = 5.0; // Hardcoded for now, perhaps better to use a getter to get it from the class file incase changes are made
        double maxTime = 20.0; // Hardcoded for now, perhaps better to use a getter to get it from the class file incase changes are made
        ...
        for (BigDecimal runningTime = start; runningTime.compareTo(end) <= 0; runningTime = runningTime.add(increment)) {
            ...
            Executable exec = () -> {
                ...
                // Inject the mock into Deca100M
                deca100M.inputResult = mockInputResult;

                // Set active to true to ensure the loop runs as expected
                deca100M.setActive(true);

                double A = deca100M.getA();
                double B = deca100M.getB();
                double C = deca100M.getC();
