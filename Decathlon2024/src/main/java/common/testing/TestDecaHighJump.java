...
public class TestDecaHighJump {
    ...
    Collection<DynamicTest> dynamicTestsForCalculateResult() {
        ...
        double maxDistance = 300.0; // Hardcoded for now, perhaps better to use a getter to get it from the class file incase changes are made

        BigDecimal start = new BigDecimal(minDistance); // "0.00"
        BigDecimal end = new BigDecimal(maxDistance); // "300.00"
        ...
        for (BigDecimal distance = start; distance.compareTo(end) <= 0; distance = distance.add(increment)) {
            ...
            Executable exec = () -> {
                ...
                decaHighJump.inputResult = mockInputResult;

                // Set active to true to ensure the loop runs as expected
                decaHighJump.setActive(true);

                double A = decaHighJump.getA();
                double B = decaHighJump.getB();
                double C = decaHighJump.getC();

                int expectedScore = calculateExpectedScore(finalDistance, A, B, C);
                int actualScore = decaHighJump.calculateResult(finalDistance);
