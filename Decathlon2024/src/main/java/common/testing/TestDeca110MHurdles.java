...
public class TestDeca110MHurdles {
    ...
    Collection<DynamicTest> dynamicTestsForCalculateResult() {
        ...
        double minTime = 10.0; // Hardcoded for now, perhaps better to use a getter to get it from the class file incase changes are made
        double maxTime = 30.0; // Hardcoded for now, perhaps better to use a getter to get it from the class file incase changes are made
        ...
        for (BigDecimal runningTime = start; runningTime.compareTo(end) <= 0; runningTime = runningTime.add(increment)) {
            ...
            Executable exec = () -> {
                ...
                // Inject the mock into Deca110MHurdles
                deca110MHurdles.inputResult = mockInputResult;

                // Set active to true to ensure the loop runs as expected
                deca110MHurdles.setActive(true);

                double A = deca110MHurdles.getA();
                double B = deca110MHurdles.getB();
                double C = deca110MHurdles.getC();

                int expectedScore = calculateExpectedScore(finalRunningTime, A, B, C);
                int actualScore = deca110MHurdles.calculateResult(finalRunningTime);
    ...
    private DynamicTest createOutOfRangeTest(String testName, double runningTime, String expectedMessage) {
        return DynamicTest.dynamicTest(testName, () -> {
            // Mock the InputResult class
            InputResult mockInputResult = Mockito.mock(InputResult.class);
            when(mockInputResult.enterResult()).thenReturn(15.0); // Provide a valid value after invalid input
