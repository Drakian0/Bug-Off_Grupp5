...
public class TestDecaPoleVault {
    ...
    Collection<DynamicTest> dynamicTestsForCalculateResult() {
        ...
        double maxDistance = 1000.0; // Hardcoded for now, perhaps better to use a getter to get it from the class file incase changes are made

        BigDecimal start = new BigDecimal(minDistance); // "0.00"
        BigDecimal end = new BigDecimal(maxDistance); // "1000.00"
        ...
        for (BigDecimal distance = start; distance.compareTo(end) <= 0; distance = distance.add(increment)) {
            ...
            Executable exec = () -> {
                ...
                // Inject the mock into DecaPoleVault
                decaPoleVault.inputResult = mockInputResult;

                // Set active to true to ensure the loop runs as expected
                decaPoleVault.setActive(true);

                double A = decaPoleVault.getA();
                double B = decaPoleVault.getB();
                double C = decaPoleVault.getC();

                int expectedScore = calculateExpectedScore(finalDistance, A, B, C);
                int actualScore = decaPoleVault.calculateResult(finalDistance);
